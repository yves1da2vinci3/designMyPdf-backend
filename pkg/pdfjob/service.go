...
func (s *Service) ProcessJob(jobID string) error {
	...
	if err != nil {
		return s.failJob(job, fmt.Sprintf("template not found: %v", err))
	}
	...
	if err != nil {
		return s.failJob(job, err.Error())
	}
...
func (s *Service) failJob(job *entities.PdfGenerationJob, errMsg string) error {
